...
authoritySchema.methods.SignAccessToken = function () {
  return jwt.sign(
    ...
    process.env.ACCESS_TOKEN || "authority",
    {
      expiresIn: "5m",
    }
  );
...
authoritySchema.methods.SignRefreshToken = function () {
  return jwt.sign(
    { id: this._id, role: this.role || "authority" },
    process.env.REFRESH_TOKEN || "authority",
    {
      expiresIn: "3d",
    }
  );
...
// compare password
authoritySchema.methods.comparePassword = async function (enteredPassword: string) {
  return await bcrypt.compare(enteredPassword, this.password);
};

const AuthorityModel: Model<IAuthority> = mongoose.model("Authority", authoritySchema);
